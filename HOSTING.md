...
`www.halilmertdeveli.com.tr` isteğe bağlı; apex çalışır, www’yi Vercel’de apex’e yönlendirebilirsin.
...
2. Ana adres: **`halilmertdeveli.com.tr`** ekle
3. İstersen **`www.halilmertdeveli.com.tr`** ekle → **Redirect to** `halilmertdeveli.com.tr`
4. İsimTescil DNS’te Vercel’in verdiği kayıtları uygula (genelde):
   - **A** `@` → `76.76.21.21`
   - **CNAME** `www` → `cname.vercel-dns.com` (veya apex’e yönlendirme)
...
- [ ] Domain → Vercel DNS (apex ana; www isteğe bağlı)  
- [ ] (Opsiyonel) Natro VPS  
